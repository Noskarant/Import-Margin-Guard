...
- **Components:** 3 KPI cards, compact comparison table, recommendation note.
- **Top KPI cards (always shown first):**
  1. **Best Scenario (Estimated Landed Cost / Unit)** — e.g. `Scenario B · €12,84 / unit`.
  2. **Savings vs Baseline** — absolute and percent, e.g. `€8 420 saved (-6,3%)`.
  3. **Estimated Gross Margin Impact** — show only if sales price exists; otherwise card shows `Not available (sales price missing)`.
- **Minimal MVP comparison table (no charts):**
  - Columns: `Scenario`, `Total Landed Cost`, `Landed Cost / Unit`, `Δ vs Baseline`, `Estimated Gross Margin %` (optional), `Confidence`.
  - Rows: one row per scenario (typically 2–3 rows).
  - Sorting: default by `Landed Cost / Unit` ascending.
- **Result summary/recommendation wording style (MVP):**
  - Practical, non-absolute language.
  - Recommended sentence pattern:  
    “Based on the assumptions entered on 22 April 2026, **Scenario B** has the lowest estimated landed cost per unit. Estimated savings vs baseline: **€8 420 (-6,3%)**.”
  - Avoid wording like “optimal”, “guaranteed”, or “system-decided”.
- **Fields:** metric selection locked to landed-cost-first in MVP (no custom KPI builder).
...
### Supported formats
- `.csv` UTF-8 (preferred, **semicolon-separated supported as first-class for France**)
...
### France-first parsing and validation defaults (MVP)
- CSV parser default tries `;` first, then `,` fallback.
- Numeric parser accepts French decimals (comma) and normalizes to dot internally:
  - accepted examples: `12,50`, `1 250,75`, `1250.75`.
- Date display defaults to `dd/MM/yyyy` in FR workspace.
- Currency display defaults to `EUR` with French formatting (e.g., `12 500,00 €`).
- Header suggestion dictionary must include common French labels:
  - `référence`, `fournisseur`, `pays d'origine`, `prix unitaire`, `quantité`, `devise`, `transport`, `droit de douane`, `incoterm`, `frais annexes`, `prix de vente`.
...
- Validation message copy defaults to French in FR orgs (EN as secondary locale).
...
### Single MVP monetization approach (no branches)
- **Free trial:** Yes, 14 days.
- **Card required at signup:** No.
- **Watermark on PDF:** No (keep reports client-credible from day 1).
- **Free access includes:** full feature access during trial, capped at:
  - 5 analyses total per organization
  - 3 PDF exports total per organization
- **Paid access (one plan only):** `Pro` at **€149/org/month** (or annual equivalent with 15% discount), includes:
  - unlimited analyses
  - unlimited PDF exports
  - up to 10 members
- **Exact upgrade trigger:** show mandatory upgrade modal when **either**:
  1. trial reaches day 14, **or**
  2. org reaches 5 analyses, **or**
  3. org reaches 3 PDF exports.
- **Post-upgrade behavior:** immediately unlock capped actions after successful Stripe checkout + webhook confirmation.
...
### Localization
- France-first defaults in MVP:
  - UI default language: French (`fr-FR`).
  - Numeric format default: decimal comma and space thousands separator (`1 234,56`).
  - Currency default: EUR (`1 234,56 €`).
  - Date default: `dd/MM/yyyy`.
  - CSV import delimiter default: semicolon (`;`), with comma fallback.
- English (`en-US`) provided as secondary locale for expansion readiness.
...
## 27. STRICT MVP IMPLEMENTATION GUARDRAIL (FOR CODEX)

### Codex must implement in v1
- Auth + organization setup (owner/admin/member only).
- CSV/XLSX upload and France-first mapping/validation.
- Scenario creation (2–3 scenarios) and landed-cost comparison view with the defined 3 KPI cards + compact table.
- Save/reopen analyses.
- PDF export.
- Single Pro billing flow with the exact trial/caps/upgrade trigger defined in Section 14.

### Codex must not implement in v1
- Viewer role, advanced role matrices, or enterprise approval workflows.
- Dashboards/charts beyond the compact comparison table.
- ERP/TMS connectors, customs automation, forecasting, AI decisioning.
- Multi-plan billing catalogs, coupons, or seat-proration logic.
- Any “nice-to-have” option not required for the core demoable workflow.

### Scope rule
If a feature is not required to complete the core workflow  
`upload -> map -> compare -> export -> save -> paywall upgrade`,  
it stays out of v1. Keep implementation narrow, practical, and demoable quickly.
